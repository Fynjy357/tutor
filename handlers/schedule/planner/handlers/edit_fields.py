...
async def planner_edit_price(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        await callback.message.edit_text(
            ...
            f"<i>Изменение цен будет применяться только к вновь запланированным занятиям. Для уже созданных занятий цена останется прежней. Если необходимо изменить её для существующего занятия, сделайте это вручную через раздел «Расписание» → «Редактировать занятие».</i>",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="❌ Отмена", callback_data=f"planner_menu_{task_id}")
            ]])
        )
